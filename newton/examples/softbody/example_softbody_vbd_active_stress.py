...
"""Compare VBD plates driven by active uniaxial stress."""
...
class Example:
    """Bend soft, medium, and stiff plates using identical active stress."""
    ...
    def step(self):
        ...
        self.solver.tet_active_stress_activations.assign(self.activation_values)
    ...
    def test_final(self):
        """Verify active stress bends the plates without instability."""
        newton.examples.test_particle_state(
            self.state_0,
            "active stress particles remain finite",
        ...
        if self.target_activation > 0.0 and np.max(tip_deflections) < 0.01:
            raise ValueError(f"Active stress did not bend the plate tips: {tip_deflections}")

    @staticmethod
    def create_parser():
        parser = newton.examples.create_parser()
        parser.set_defaults(num_frames=180)
        parser.add_argument("--activation", type=float, default=0.6)
        parser.add_argument("--active-stress", type=float, default=2.0e4)
