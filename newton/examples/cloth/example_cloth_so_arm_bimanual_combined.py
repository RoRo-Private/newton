...
#
# Command: uv run -m newton.examples cloth_so_arm_bimanual_combined
#          uv run -m newton.examples cloth_so_arm_bimanual_combined --webxr
#
###########################################################################
...
_SO_ARM_BIMANUAL_URDF = (
    Path(__file__).resolve().parents[3] / "so_arm_description" / "so101_bimanual_set" / "so101_bimanual_set.urdf"
)

_CLOTH_DATA_DEFAULT = (
    Path(__file__).resolve().parents[3] / "cloth_data" / "cloth_1_2" / "newton_cloth_hole_2" / "cloth_export.npz"
)
...
_HOME_POSE[4] = -90.0 * np.pi / 180.0

_HOME_POSE[7] = 0.0 * np.pi / 180.0
_HOME_POSE[9] = 90.0 * np.pi / 180.0
_HOME_POSE[10] = -90.0 * np.pi / 180.0

# ---------------------------------------------------------------------------
# Gripper camera mount parameters (tune these to match physical camera).
# All offsets/directions are in the LOCAL frame of the respective link.
# ---------------------------------------------------------------------------
# Position offset from gripper_link origin (in gripper_link local frame [m])
_GRIPPER_CAM_POS_Y  =  0.06   # along local Y axis
_GRIPPER_CAM_POS_Z  = -0.04   # along local Z axis
# Forward direction base: -X axis of gripper_frame_link local frame
# Tilt: rotates the base forward along local Z of gripper_frame_link [deg]
# Negative = tilt downward (toward -local-Z), positive = upward
_GRIPPER_CAM_TILT_DEG = 45.0

EPISODE_DURATION = 36.0  # [s] default teleoperation episode length
_WAYPOINT_YAW_OFFSET_RAD = 0.0  # +90 deg around world Z
_WAYPOINT_QUAT_IS_WXYZ = True
_IK_FRAME_ROLL_OFFSET_RAD = 0.0   # around local X (gripper frame)
_IK_FRAME_PITCH_OFFSET_RAD = np.pi # around local Y
_IK_FRAME_YAW_OFFSET_RAD = 0     # around local Z

# Waypoint targets imported from the motion variant.
LEFT_INIT_POS_XYZ = (0.15, 0.15, 0.1)
RIGHT_INIT_POS_XYZ = (0.15, -0.15, 0.1)
LEFT_INIT_QUAT_XYZW = (-0.5, -0.5, -0.5, 0.5)
RIGHT_INIT_QUAT_XYZW = (-0.5, -0.5, -0.5, 0.5)
...
WAYPOINTS = [
    {
        ...
        "r_ee_target_quat": (-0.6123724, -0.3535534, -0.6123724, 0.3535534),
        "l_gripper_cmd": 0.8,
        "r_gripper_cmd": 0.8,
        "move_duration": 1.5,
    },
    ...
    },
    {
        ...
        "r_ee_target_quat": (-0.3535534, -0.6123724, -0.3535534, 0.6123724),
        "l_gripper_cmd": 0.8,
        "r_gripper_cmd": 1.0,
        "move_duration": 3.0,
    },
    {
        "l_ee_target_pose": LEFT_INIT_POS_XYZ,
        "l_ee_target_quat": LEFT_INIT_QUAT_XYZW,
        "r_ee_target_pose": (0.230, 0.115, 0.15),
        "r_ee_target_quat": (-0.3535534, -0.6123724, -0.3535534, 0.6123724),
        "l_gripper_cmd": 0.8,
        "r_gripper_cmd": 0.5,
        "move_duration": 1.5,
    ...
    },
    {
        ...
        "l_ee_target_quat": (-0.3535534, -0.6123724, -0.3535534, 0.6123724),
        "r_ee_target_pose": RIGHT_INIT_POS_XYZ,
        "r_ee_target_quat": RIGHT_INIT_QUAT_XYZW,
        "l_gripper_cmd": 0.8,
        "r_gripper_cmd": 0.8,
    ...
    {
        "l_ee_target_pose": (0.225, 0.3, 0.15),
        "l_ee_target_quat": (-0.3535534, -0.6123724, -0.3535534, 0.6123724),
        "r_ee_target_pose": RIGHT_INIT_POS_XYZ,
        "r_ee_target_quat": RIGHT_INIT_QUAT_XYZW,
        "l_gripper_cmd": 1.0,
    ...
    },
    {
        ...
        "l_ee_target_quat": (-0.6123724, -0.3535534, -0.6123724, 0.3535534),
        "r_ee_target_pose": RIGHT_INIT_POS_XYZ,
        "r_ee_target_quat": RIGHT_INIT_QUAT_XYZW,
        "l_gripper_cmd": 1.0,
        "r_gripper_cmd": 0.8,
    ...
    {
        "l_ee_target_pose": (0.24, -0.105, 0.15),
        "l_ee_target_quat": (-0.6123724, -0.3535534, -0.6123724, 0.3535534),
        "r_ee_target_pose": RIGHT_INIT_POS_XYZ,
        "r_ee_target_quat": RIGHT_INIT_QUAT_XYZW,
        "l_gripper_cmd": 0.5,
    ...
    {
        "l_ee_target_pose": (0.25, 0.1, 0.1),
        "l_ee_target_quat": LEFT_INIT_QUAT_XYZW,
        "r_ee_target_pose": (0.25, -0.08, 0.1),
        "r_ee_target_quat": RIGHT_INIT_QUAT_XYZW,
        "l_gripper_cmd": 0.7,
        "r_gripper_cmd": 0.7,
        "move_duration": 1.5,
    ...
    {
        "l_ee_target_pose": (0.25, 0.1, 0.0),
        "l_ee_target_quat": LEFT_INIT_QUAT_XYZW,
        "r_ee_target_pose": (0.25, -0.08, 0.0),
        "r_ee_target_quat": RIGHT_INIT_QUAT_XYZW,
        "l_gripper_cmd": 0.7,
        "r_gripper_cmd": 0.7,
        "move_duration": 1.5,
    ...
    {
        "l_ee_target_pose": (0.25, 0.09, 0.0),
        "l_ee_target_quat": LEFT_INIT_QUAT_XYZW,
        "r_ee_target_pose": (0.25, -0.09, 0.0),
        "r_ee_target_quat": RIGHT_INIT_QUAT_XYZW,
        "l_gripper_cmd": 1.0,
        "r_gripper_cmd": 1.0,
        "move_duration": 1.0,
    ...
    {
        "l_ee_target_pose": (0.1, 0.09, 0.15),
        "l_ee_target_quat": LEFT_INIT_QUAT_XYZW,
        "r_ee_target_pose": (0.1, -0.09, 0.15),
        "r_ee_target_quat": RIGHT_INIT_QUAT_XYZW,
        "l_gripper_cmd": 1.0,
        "r_gripper_cmd": 1.0,
        "move_duration": 3.0,
    ...
    {
        "l_ee_target_pose": (0.40, 0.09, 0.05),
        "l_ee_target_quat": LEFT_INIT_QUAT_XYZW, #(-0.2705981, -0.6532815, -0.6532815, 0.2705981),
        "r_ee_target_pose": (0.40, -0.09, 0.05),
        "r_ee_target_quat": RIGHT_INIT_QUAT_XYZW, #(-0.2705981, -0.6532815, -0.6532815, 0.2705981),
        "l_gripper_cmd": 1.0,
        "r_gripper_cmd": 1.0,
        "move_duration": 3.0,
    },
    {
        "l_ee_target_pose": (0.35, 0.10, 0.1),
        "l_ee_target_quat": LEFT_INIT_QUAT_XYZW, #(-0.2705981, -0.6532815, -0.6532815, 0.2705981),
        "r_ee_target_pose": (0.35, -0.08, 0.1),
        "r_ee_target_quat": RIGHT_INIT_QUAT_XYZW, #(-0.2705981, -0.6532815, -0.6532815, 0.2705981),
        "l_gripper_cmd": 0.5,
        "r_gripper_cmd": 0.5,
        "move_duration": 1.5,
...
class _FfmpegVideoWriter:
    """Write raw RGB frames to an MP4 via an ffmpeg stdin pipe.

    Frames are written synchronously; ffmpeg encodes asynchronously in a
    child process.  Call ``close()`` (or let ``atexit`` handle it) to flush
    the encoder and finalise the file.
    """
    ...
    def write(self, frame: np.ndarray) -> None:
        """Write one (H, W, 3) uint8 RGB frame."""
    ...
    def close(self) -> None:
        if self._proc.stdin:
            try:
                ...
                pass
        self._proc.wait()
        print(f"[Capture] Video saved: {self.path}")
...
class Example:
    def __init__(self, viewer, args=None):
        ...
        self._enable_rigid_contacts = False  # false if don't care about robot <-> table contact

        # Cloth elasticity
        self.tri_ke = 1e4
        self.tri_ka = 1e4
        self.tri_kd = 1.5e-6
        self.bending_ke = 10.0
        self.bending_kd = 1e-2

        # Cloth contact
        self.shape_material_mu = 1.0
        ...
        scene.add_shape_plane(
            body=-1,
            xform=wp.transform(wp.vec3(0.3, 0.0, 0.02), wp.quat_identity()),
            width=1.0,
            length=3.0,
            cfg=floor_cfg,
        )
        ...
        # Solvers
        # ------------------------------------------------------------------
        self._robot_solver = SolverMuJoCo(self.model)
        ...
        # ------------------------------------------------------------------
        self._capture_cameras_enabled = args is not None and getattr(args, "capture_cameras", False)
        self._capture_every = max(1, int(getattr(args, "capture_every", 1))) if args is not None else 1
        self._capture_dir = Path(getattr(args, "capture_dir", "camera_capture")) if args is not None else Path("camera_capture")
        self._capture_counter = 0
        self._video_writers: dict[str, _FfmpegVideoWriter] = {}
        self._meta_frames: list[dict] = []
        if self._capture_cameras_enabled:
            self._capture_dir.mkdir(parents=True, exist_ok=True)
            cap_fps = self.fps / self._capture_every
            self._video_writers["main"]          = _FfmpegVideoWriter(self._capture_dir / "main.mp4",          640, 480, cap_fps)
            self._video_writers["gripper_left"]  = _FfmpegVideoWriter(self._capture_dir / "gripper_left.mp4",  224, 224, cap_fps)
            self._video_writers["gripper_right"] = _FfmpegVideoWriter(self._capture_dir / "gripper_right.mp4", 224, 224, cap_fps)
            import atexit  # noqa: PLC0415
            atexit.register(self._finalize_capture)
            print(
                f"[SO-ARM Capture] Enabled: dir={self._capture_dir}, "
                f"every={self._capture_every} frame(s), fps={cap_fps:.1f}"
            )

        # Camera frustum visualization (optional)
        self._show_cameras = args is not None and getattr(args, "show_cameras", False)
    ...
    def _get_current_gripper_frame_transforms(self) -> tuple[wp.transform, wp.transform]:
        ...
        left_q = R.from_matrix(left_T[:3, :3]).as_quat()
        right_q = R.from_matrix(right_T[:3, :3]).as_quat()
    ...
    def _get_waypoint_segment(self, t: float):
        def _wp_pose(pose_key: str, quat_key: str, wp_dict: dict, fallback_quat: tuple[float, ...]):
            p = np.array(wp_dict[pose_key], dtype=np.float64)
            q_raw = np.array(wp_dict.get(quat_key, fallback_quat), dtype=np.float64)
    ...
    def render(self) -> None:
        ...
        if self._show_cameras:
            self._log_camera_frustums()

        self.viewer.end_frame()
        self._capture_cameras_if_needed()

    # ------------------------------------------------------------------
    # Camera frustum visualization
    # ------------------------------------------------------------------
    @staticmethod
    def _gripper_cam_pose(
        T_link: np.ndarray,
        T_frame: np.ndarray,
    ) -> tuple[np.ndarray, np.ndarray]:
        """Compute gripper camera position and forward vector.

        Both are expressed entirely in the local frames of gripper_link /
        gripper_frame_link, so they rotate rigidly with the arm.

        Returns:
            pos: world-space camera position [3]
            fwd: normalized world-space forward direction [3]
        """
        # Position: fixed offset in gripper_link local frame
        pos = T_link[:3, 3] + T_link[:3, :3] @ np.array([0.0, _GRIPPER_CAM_POS_Y, _GRIPPER_CAM_POS_Z])

        # Forward: base (-X of gripper_frame_link) + tilt along local Z,
        # all expressed in gripper_frame_link local frame → transform to world.
        tilt_rad = np.radians(_GRIPPER_CAM_TILT_DEG)
        local_fwd = np.array([-1.0, 0.0, 0.0]) + np.array([0.0, -1.0, 0.0]) * np.tan(tilt_rad)
        fwd = T_frame[:3, :3] @ local_fwd
    ...
    def _frustum_lines(
        ...
        """Return (starts, ends) arrays [N,3] for a camera frustum.

        Draws 4 rays from origin to frustum corners + rectangle at *depth*.
        Args:
            pos: camera position [3]
            fwd: normalized forward direction vector [3]
            fov_deg: vertical field of view in degrees
            aspect: width/height aspect ratio
            depth: frustum visualization depth [m]
        """
        fwd = fwd / (np.linalg.norm(fwd) + 1e-8)

        # Build camera-frame right/up from fwd, without going through angles
        world_up = np.array([0.0, 0.0, 1.0])
        right = np.cross(fwd, world_up)
        if np.linalg.norm(right) < 1e-6:
            # fwd is nearly vertical — use X as fallback up
        ...
        half_v = np.tan(np.radians(fov_deg * 0.5))
        half_h = half_v * aspect

        # 4 frustum corners at distance *depth*
        corners = np.array([
            fwd * depth + right * half_h * depth + up * half_v * depth,
            fwd * depth - right * half_h * depth + up * half_v * depth,
            fwd * depth - right * half_h * depth - up * half_v * depth,
            fwd * depth + right * half_h * depth - up * half_v * depth,
        ]) + pos
        ...
        o = pos
        # 4 origin→corner rays + 4 rectangle edges
        starts = np.array([o, o, o, o,
                           corners[0], corners[1], corners[2], corners[3]])
        ends   = np.array([corners[0], corners[1], corners[2], corners[3],
                           corners[1], corners[2], corners[3], corners[0]])
        return starts, ends

    def _log_camera_frustums(self) -> None:
        """Visualize all virtual cameras as frustum wireframes."""
        ...
        # --- Main (overhead D455) ---
        _pitch, _yaw = np.radians(-60.0), np.radians(0.0)
        _fwd_main = np.array([
            np.cos(_yaw) * np.cos(_pitch),
            np.sin(_yaw) * np.cos(_pitch),
            np.sin(_pitch),
        ])
        s, e = self._frustum_lines(
            pos=np.array([0.0, 0.0, 0.52]),
            fwd=_fwd_main,
            fov_deg=58.0,
            aspect=640 / 480,
            depth=0.20,
        ...
        self.viewer.log_lines(
            "/cam_main",
            wp.array(s, dtype=wp.vec3, device=dev),
            wp.array(e, dtype=wp.vec3, device=dev),
            (0.2, 0.8, 1.0),  # cyan
            width=0.004,
        )

        # --- Gripper cameras (dynamic, needs placo FK) ---
        if not hasattr(self, "_placo_robot_left"):
            return
        ...
        left_T_link  = self._placo_robot_left.get_T_world_frame("left_gripper_link")
        right_T_link = self._placo_robot_right.get_T_world_frame("right_gripper_link")
        left_T_frame  = self._placo_robot_left.get_T_world_frame("left_gripper_frame_link")
        right_T_frame = self._placo_robot_right.get_T_world_frame("right_gripper_frame_link")

        for T_link, T_frame, cam_name, color in (
            (left_T_link,  left_T_frame,  "/cam_gripper_left",  (1.0, 0.8, 0.2)),  # yellow
            (right_T_link, right_T_frame, "/cam_gripper_right", (1.0, 0.4, 0.2)),  # orange
        ):
            pos_g, fwd_g = self._gripper_cam_pose(T_link, T_frame)
            s, e = self._frustum_lines(
                pos=pos_g,
                fwd=fwd_g,
                fov_deg=95.0,
    ...
    def _capture_cameras_if_needed(self) -> None:
        if not self._capture_cameras_enabled:
            return
        if not hasattr(self.viewer, "renderer") or not hasattr(self.viewer, "camera"):
            return

        if self._capture_counter % self._capture_every == 0:
            self._save_camera_triplet()
        self._capture_counter += 1

    def _render_and_save(self, cam, name: str) -> None:
        """Render and write a frame to the named video writer.

        get_frame() always returns the window framebuffer size, so the frame is
        resized to the writer's declared dimensions before piping to ffmpeg.
        """
        from PIL import Image  # noqa: PLC0415

        self.viewer.renderer.render(cam, self.viewer.objects, self.viewer.lines)
        frame = self.viewer.get_frame().numpy()  # (screen_H, screen_W, 3) uint8
        writer = self._video_writers.get(name)
        if writer is not None:
            if frame.shape[0] != writer.height or frame.shape[1] != writer.width:
                frame = np.array(
                    Image.fromarray(frame).resize((writer.width, writer.height), Image.LANCZOS)
                )
            writer.write(frame)

    def _save_camera_triplet(self) -> None:
        """Write main + gripper-left + gripper-right frames to their video writers
        and accumulate per-frame metadata."""
        ...
        cam.width = 640
        cam.height = 480
        self._render_and_save(cam, "main")

        # Gripper cameras: webcam-like setup.
        q_np = self._ik_joint_q.numpy()
        for i, jname in enumerate(self._left_joint_names):
            self._placo_robot_left.set_joint(jname, float(q_np[i]))
        self._placo_robot_left.update_kinematics()
        for i, jname in enumerate(self._right_joint_names):
            self._placo_robot_right.set_joint(jname, float(q_np[_N_COORDS_PER_ARM + i]))
        self._placo_robot_right.update_kinematics()
        ...
        cam.width = 224
        cam.height = 224
        for T_link, T_frame, vname in (
            (left_T_link,  left_T_frame,  "gripper_left"),
            (right_T_link, right_T_frame, "gripper_right"),
        ):
            pos, fwd = self._gripper_cam_pose(T_link, T_frame)
            pitch = float(np.degrees(np.arcsin(np.clip(fwd[2], -1.0, 1.0))))
            yaw   = float(np.degrees(np.arctan2(fwd[1], fwd[0])))
            cam.pos = PyVec3(*pos.tolist())
            cam.pitch = pitch
            cam.yaw = yaw
            self._render_and_save(cam, vname)

        # Accumulate metadata (saved as a single file in _finalize_capture).
        self._meta_frames.append({
            "sim_time": float(self.sim_time),
            "state": self.get_obs_state().copy() if hasattr(self.get_obs_state(), "copy") else self.get_obs_state(),
            "action": self.get_action().copy() if hasattr(self.get_action(), "copy") else self.get_action(),
        })
    ...
    def _finalize_capture(self) -> None:
        """Close all video writers and flush accumulated metadata to a single npz file."""
        ...
        if self._meta_frames:
            np.savez_compressed(
                self._capture_dir / "meta.npz",
                sim_time=np.array([f["sim_time"] for f in self._meta_frames], dtype=np.float64),
                state=np.array([f["state"]    for f in self._meta_frames]),
                action=np.array([f["action"]  for f in self._meta_frames]),
            )
            print(f"[Capture] Metadata ({len(self._meta_frames)} frames) → {self._capture_dir / 'meta.npz'}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--capture-cameras", action="store_true", help="Capture and save main/gripper camera images.")
    parser.add_argument("--capture-dir", type=str, default="camera_capture", help="Directory to save camera captures.")
    parser.add_argument("--capture-every", type=int, default=1, help="Save one capture every N rendered frames.")
    parser.add_argument("--show-cameras", action="store_true", help="Visualize camera frustums in the viewer.")
