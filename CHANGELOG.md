...
- Add an `artificial_muscle_joint` example that visualizes a black gabled BCC CFRP lattice frame.
...
- Add a `softbody_tet_activation` example comparing active bending across three tetrahedral plate materials.
...
- Add per-tetrahedron directional active contraction energy to `SolverVBD` and a three-material plate-bending example.
- Add per-tetrahedron active stress and active strain controls to `SolverVBD`, with three-material plate-bending examples for each mode.
- Add fixed-end VBD strip and cylinder linear contraction examples with Poisson-style transverse active strain.
